...
public final class StartNodeMock {
	...
	private static final class TruncatedDefaultStartNodeBuilder<T extends StartNode> extends DefaultStartNodeBuilder<T, TruncatedStartNodeBuilder<T>, TruncatedDefaultStartNodeBuilder<T>> implements TruncatedStartNodeBuilder<T> {
		...
		<OT extends SiteStoreFolder, OTBUILDER extends SiteStoreFolderBuilder<OT, OTBUILDER>> TruncatedDefaultStartNodeBuilder(String uid, long id, UidType uidType, OTBUILDER parent) {
			super(uid, id, uidType, parent);
		}

		<OTBUILDER extends StartNodeBuilder<T, OTBUILDER>> TruncatedDefaultStartNodeBuilder(OTBUILDER startNode) {
			super(startNode);
		}
	}
}
